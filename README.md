...
<p align="center">
  <img src="screenshots/header-banner.jpg" alt="A portrait generated by AI Writing Tools' in-process SDXL Image Gen" width="360">
</p>
...
![Python](https://img.shields.io/badge/python-3.13+-blue) ![PyQt6](https://img.shields.io/badge/PyQt6-6.5+-green) ![Version](https://img.shields.io/badge/version-1.10.2-violet) ![License](https://img.shields.io/badge/license-MIT-purple) ![Platform](https://img.shields.io/badge/platform-Windows-lightgrey)
...
**Forge Neo** *(see deep-dive section below)*
- Start / Stop / Open buttons — same shape as SillyTavern (an external directory + launch script), separate from the in-process Image Gen backend above

**Kill All**
- Force-terminates KoboldCpp, SillyTavern, and Forge Neo in one click
- Useful when processes get stuck or you want a clean reset

**Restart App**
- Next to Kill All — stops every managed service (with the same in-flight-generation confirmation as closing the window) and relaunches the app fresh, so config changes that need a restart don't mean manually closing and reopening it
...
- **Forge Neo tab** — directory path, port, a free-text launch-args field (e.g. `--xformers --cuda-malloc --sage`), and a combined **Check for Updates** button that shows current/available version and updates in one click
...
A managed connection to a full-featured **[Forge Neo](https://github.com/Haoming02/sd-webui-forge-classic/tree/neo)** install — separate from the in-process Image Gen backend above by design, not a replacement for it. Image Gen stays the fast, curated, always-available path for card portraits; Forge Neo is for running whatever state-of-the-art model the curated pipeline doesn't support (Krea 2's DiT architecture, for example), with the full extension/preset ecosystem that comes with a real Forge install.

### Setup

This app doesn't install or manage Forge Neo's own Python environment — that's Forge's own `webui-user.bat` bootstrap process (creates its venv, installs `torch`/requirements). Point Settings → Forge Neo's directory field at a Forge Neo checkout that's already been run once by hand, so its venv exists. From then on, this app launches the venv's `python.exe launch.py` directly (same pattern as SillyTavern's `node.exe server.js` — no batch-file wrapper involved).

### Start / Stop / Open

- **Start** launches Forge Neo with whatever's in the Launch args field, watches its log for the ready signal, and enables Open once it's up
- **Stop** / **Kill All** / **Restart App** all tear it down the same reliable way as KoboldCpp/SillyTavern — a Job Object guarantees cleanup even if this app itself crashes
- **Open** launches your browser to Forge Neo's UI

### Launch args

A free-text field (Settings → Forge Neo), not a fixed checkbox list — Forge Neo's flag surface is large and changes upstream. Common useful ones:
- `--xformers` — memory-efficient attention
- `--cuda-malloc` — faster CUDA memory allocator
- `--sage` — SageAttention 2, can meaningfully outrun `--xformers` alone on tensor-core-heavy GPUs, at the cost of needing an exact-matched Triton build (Forge Neo installs it automatically the first time `--sage` runs)

### Check for Updates

A single combined check-and-update button (Settings → Forge Neo), not a separate check-then-update pair:
- Reads the current and remote-available version straight from Forge Neo's own version file (the same string its "Version: neo 2.27"-style startup line comes from)
- Fetches and compares before deciding whether to pull, so a rejected fast-forward (e.g. local edits) still reports "update available X → Y" instead of a bare git error
- Deliberately fast-forward-only — never creates a merge commit or silently resolves a divergence
- Doesn't auto-restart Forge Neo afterward; Stop then Start to pick up an update
...
- **[Forge Neo](https://github.com/Haoming02/sd-webui-forge-classic/tree/neo)** (optional — a separate checkout, entirely outside this app's own venv/requirements) — only needed if you want the full Forge model playground on top of the in-process Image Gen backend. Run its own `webui-user.bat` once by hand first so its venv exists; see the Forge Neo deep-dive section above.
...
```json
{
    ...
    "forge": {
        "dir":  "C:\\path\\to\\Forge Neo",
        "port": 7861,
        "args": "--xformers --cuda-malloc --sage"
    },
    "api": {
...
- `forge` — the entire block is optional; leave `dir` empty (or the block out) to hide Forge Neo entirely. `port` defaults to **7861**, not 7860 — `sdxl.port` below already defaults to 7860 for the in-process Image Gen API server, and a colliding default would make both fight over the same port. `args` is free-text, space-separated Forge Neo launch flags.
...
**Forge Neo:**
1. Install Forge Neo separately and run its `webui-user.bat` once by hand so its venv exists
2. Point Settings → Forge Neo's directory field at that checkout, set launch args if you want any (`--xformers --cuda-malloc --sage`, etc.)
3. Click **Start** on the Forge Neo row, then **Open** once it's ready
4. Use Forge's own UI as you would any Forge install — models, extensions, and presets it exposes are entirely up to that install
5. Settings → Forge Neo → **Check for Updates** pulls the latest `neo` branch in one click when you want it
